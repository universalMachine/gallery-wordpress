...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'acme-col-3 masonry-post' ); ?>>
	...
	if(has_post_content()):
		...
		$image_url[0] = get_template_directory_uri().'/assets/img/no-image-840-480.jpg';
		//	$image_url[0] = wp_get_content_image_src( get_post_thumbnail_id(), $infinite_photography_blog_archive_image_size );
		//	$image_url_full[0] = wp_get_content_image_src( get_post_thumbnail_id(), $infinite_photography_blog_archive_image_size );
		$image_url_full[0] = get_template_directory_uri().'/assets/img/no-image-840-480.jpg';
	endif;
	?>
